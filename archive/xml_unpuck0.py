...
class Owner:
	...
	def __init__(self, right_node):
		...
		self.share_numenator = right_node.find('.//Registration/Share').get('Numerator') if right_node.find('.//Registration/Share') is not None else '1'
		self.share_denuminator = right_node.find('.//Registration/Share').get('Denominator') if right_node.find('.//Registration/Share') is not None else '1'
	
	def __repr__(self):
		return "{} - {}\{}".format(self.fio, self.share_numenator, self.share_denuminator)

	def set_vouts_count(vouts):
		self.vouts = vouts
...
class Responce:
	...
	def __init__(self, path):
		...
		self.area = root.find('.//Area/Area').text
		...
		self.filename = self.apartment if self.apartment != 'None' else  self.area + '-' + self.c_num.replace(':', '!')
...
# def make_resp_note(resp_obj):
# 	count = len(resp_obj.owner_objs)
# 	for owner in resp_obj.owner_objs:
# 		vouts_count = resp.area / owner.share_denuminator * owner.share_numenator
...
if __name__ == '__main__':
	...
	print(test)
	print(test.owner_objs[0].regnumber, test.area)
	print('\n\n')
	...
	l = len([[print("{}, {}/{}".format(resp.area, o_obj.share_numenator, o_obj.share_denuminator)) for o_obj in resp.owner_objs] for resp in test2])
	#print(l)
	
	# [resp.copy_to_addr(res_folder) for resp in test2 ]
